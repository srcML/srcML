 ...
 */
int srcml_extract_text(const char * input_buffer, size_t size, UTF8OutputSource & output_handler, const char * encoding, OPTION_TYPE options, int unit) {

    if(input_buffer == NULL || size == 0) return SRCML_STATUS_ERROR;

    xmlParserInputBufferPtr input = xmlParserInputBufferCreateMem(input_buffer, (int)size, xmlParseCharEncoding(encoding));

    if(input == NULL) return SRCML_STATUS_ERROR;

    srcMLSAX2Reader reader(input, encoding);
...
int srcml_extract_text_filename(const char * ifilename, const char * ofilename, const char * encoding, OPTION_TYPE options, int unit) {
    ...
    UTF8OutputSource output_handler(ofilename, encoding, "UTF-8");
