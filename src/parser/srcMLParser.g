...
function_type[int type_count] { ENTRY_DEBUG } :
        ...
        (options { generateAmbigWarnings = false; } : auto_keyword[type_count > 1] | ( { getTypeCount() > 1 }? pure_lead_type_identifier { decTypeCount(); })* lead_type_identifier | { inLanguage(LANGUAGE_JAVA) }? default_specifier)
