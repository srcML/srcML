...
// creates the source-code file in the in unit in the given filename path
static void create_file(int, std::shared_ptr<srcml_unit> unit, std::filesystem::path path) {
...
void src_output_filesystem(srcml_archive* srcml_arch, std::string_view output_dir, TraceLog& log) {
    ...
    int count = 0;
    while (std::shared_ptr<srcml_unit> unit{srcml_archive_read_unit(srcml_arch)}) {
        ...
        // create the file using a thread pool
        pool.push(create_file, std::move(unit), path);
