...
void option_command_deprecated(bool opt) {
    if (opt) {
      ...
      if (command == SRCML_COMMAND_UNITS)
        SRCMLlog(INFO_MSG, "srcml: use of option --units or -n is deprecated");
      if (command == SRCML_COMMAND_EXPRESSION)
        SRCMLlog(INFO_MSG, "srcml: use of option --expression or -e is deprecated");
    }
}
...
/*
    if (value.empty() || srcml_check_encoding(value.c_str()) == 0) {
        SRCMLlog(ERROR_MSG, "srcml: invalid src encoding \"" + value + "\"");
...
void option_field<&srcml_request_t::att_xml_encoding>(const std::string& value) {
    ...
    if (value.empty() || srcml_check_encoding(value.c_str()) == 0) {
        SRCMLlog(ERROR_MSG, "srcml: invalid xml encoding \"%s\"", value);
        exit(CLI_ERROR_INVALID_ARGUMENT);
    }
...
void option_field<&srcml_request_t::att_language>(const std::string& value) {
    ...
    if (value.empty() || srcml_check_language(value.c_str()) == 0) {
        SRCMLlog(ERROR_MSG, "srcml: invalid language \"%s\"", value);
...
void option_field<&srcml_request_t::tabs>(int value) {
    ...
    // check tabstop
    if (value < 1) {
        SRCMLlog(ERROR_MSG, "srcml: %d is an invalid tab stop. Tab stops must be 1 or higher.", value);
...
void option_xmlns_prefix(const std::vector<std::string>& values) {
    for (const auto& value : values )
    {
      ...
      if (delim == std::string::npos) {
        SRCMLlog(ERROR_MSG, "srcml: xmlns format missing \"=\"");
        exit(1); //ERROR CODE TBD
      }
...
srcml_request_t parseCLI(int argc, char* argv[]) {
    try {
          ...
          if (option.string_key == "relaxng" || option.string_key == "xpath" || option.string_key == "xslt" || option.string_key == "xpathparam"
            ...
            for (const auto& vals : option.value) {
              if (option.string_key == "element" && srcml_request.xpath_query_support.size() < 1) {
                ...
                SRCMLlog(ERROR_MSG, "srcml: element option must follow an --xpath option");
                exit(SRCML_STATUS_INVALID_ARGUMENT);
              }
              if (option.string_key == "attribute" && srcml_request.xpath_query_support.size() < 1) {

                SRCMLlog(ERROR_MSG, "srcml: attribute option must follow an --xpath option");
                exit(SRCML_STATUS_INVALID_ARGUMENT);
              }
        ...
        for (const auto& ns : reserved_namespaces) {
          ...
          SRCMLlog(ERROR_MSG, "srcml: prefix \"" + prefix + "\" assigned multiple URIs \"" + uri + "\", \"" + srcml_request.xmlns_namespaces[prefix] + "\"");
    ...
    // Unknown Option
        ...
        SRCMLlog(ERROR_MSG, "srcml: %s", e.what());
        exit(3);
    }
    // Missing Option Value
        ...
        SRCMLlog(ERROR_MSG, "srcml: %s", error_msg);
        exit(7);
    }
...
void conflicting_options(const prog_opts::variables_map& vm, const char* opt1, const char* opt2) {
    if (vm.count(opt1) && !vm[opt1].defaulted() && vm.count(opt2) && !vm[opt2].defaulted()) {
        SRCMLlog(ERROR_MSG, "srcml: Conflicting options '%s' and '%s'.", opt1, opt2);
        exit(15);
    }
}
...
attribute clean_attribute_input(const std::basic_string< char >& attribute_input) {
  ...
  if (attrib_equals == std::string::npos) {
    SRCMLlog(ERROR_MSG, "srcml: the attribute %s is missing a value", vals);
    exit(SRCML_STATUS_INVALID_ARGUMENT);
  }
  ...
  if (attrib_colon == std::string::npos && !(srcml_request.xpath_query_support[srcml_request.xpath_query_support.size() - 1].first)) {
    SRCMLlog(ERROR_MSG, "srcml: the attribute %s is missing a prefix or an element with a prefix", vals);
    exit(SRCML_STATUS_INVALID_ARGUMENT);
  }
