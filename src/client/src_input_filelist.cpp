...
int src_input_filelist(ParseQueue& queue,
    ...
    // Use a stream for robust line-by-line parsing, handling various line endings correctly
    ...
    while (std::getline(iss, sline)) {
        ...
        // verify that the file entry is not the same as the file list itself to avoid infinite recursion
        std::error_code ec;
        ...
        if (fileStatus == -1) {
            // Propagate failure to ensure non-zero exit code on errors
            return -1;
        }
    }
