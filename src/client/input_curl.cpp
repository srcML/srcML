...
int input_curl(srcml_input_src& input) {
    ...
    // On Windows, the asynchronous pipe-based model used by libcurl can be fragile.
    // Instead, we use a simpler synchronous model: download the URL to a temporary file
    // and then provide that file handle to the rest of the application.
    ...
    // Set up CURL options for a robust download
    ...
    // Flush and rewind the temporary file so it's ready for reading from the beginning
    fflush(temp_file);
    rewind(temp_file);
    ...
    return 1;
#else
    // For Unix-like systems, continue using the pipe-based model
